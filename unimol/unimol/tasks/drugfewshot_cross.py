...
class DrugCLIP(UnicoreTask):
    ...
    def test_pcba_target(self, target, model, **kwargs):
        ...
        # data_path_train = f"/root/autodl-tmp/Drug-fewshot/data/lit_pcba/{target}/mols_few{ft}_scaffold{suffix}.lmdb"
        # data_path_test = f"/root/autodl-tmp/Drug-fewshot/data/lit_pcba/{target}/mols_remain{ft}_scaffold{suffix}.lmdb"
        ...
        # optimizer = unicore.optim.adam.Adam(params, lr=0.001)
        # optimizer = unicore.optim.adam.Adam(model.parameters(), lr=0.001)
        # loss_fn = torch.nn.BCEWithLogitsLoss()
        epoch = self.args.epoch_train
        mol_rep_map = None
        rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/pcba/{target}.npz.pkl.gz"
        pocket_rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/pcba/{target}.pockets.npz.pkl.gz"
    
        with gzip.open(rep_path, "rb") as f:
            mol_rep_map = pickle.load(f)
        with gzip.open(pocket_rep_path, "rb") as f:
            pocket_rep_map = pickle.load(f)
        ...
        for i in tqdm(range(epoch)):
            for _, sample_pocket in enumerate(pocket_data):
                ...
                pocket_rep_ori = None
                if pocket_rep_map is not None:
                    feats = []
                    for name in smi_names:
                        if name in pocket_rep_map:
                            target_length = pocket_st.size(1)
                            actual_length = len(pocket_rep_map[name])


                            feat = np.zeros((target_length, pocket_rep_map[name].shape[1]), dtype=np.float32)

                            if actual_length > 0:
                                if actual_length >= target_length:
                                    feat = np.asarray(pocket_rep_map[name][:target_length], dtype=np.float32)
                                else:
                                    feat[:actual_length] = np.asarray(pocket_rep_map[name], dtype=np.float32)
                                    feat[actual_length:] = feat[actual_length - 1:actual_length]
                    ...
                    if feats is not None:
                        try:
                            pocket_rep_ori = torch.from_numpy(np.stack(feats)).to(pocket_st.device)
                        except Exception:
                            pocket_rep_ori = None
                for _, sample in enumerate(mol_data_train):
                        ...
                        mol_rep_ori = None
                        if mol_rep_map is not None:
                            feats = []
                            for name in smi_names:
                                if name in mol_rep_map:
                                 
                                    target_length = mol_st.size(1)
                                    actual_length = len(mol_rep_map[name])

                                    feat = np.zeros((target_length, mol_rep_map[name].shape[1]), dtype=np.float32)

                                    if actual_length > 0:
                                        if actual_length >= target_length:
                                            feat = np.asarray(mol_rep_map[name][:target_length], dtype=np.float32)
                                        else:
                                            feat[:actual_length] = np.asarray(mol_rep_map[name], dtype=np.float32)
                                            feat[actual_length:] = feat[actual_length - 1:actual_length]
                            ...
                            if feats is not None:
                                try:
                                    mol_rep_ori = torch.from_numpy(np.stack(feats)).to(mol_st.device)
                                except Exception:
                                    mol_rep_ori = None
            ...
            for _, sample in enumerate(tqdm(mol_data_test)):
                ...
                if mol_rep_map is not None:
                    ...
                    for name in smi_names:
                        if name in mol_rep_map:
                            target_length = mol_st.size(1)
                            arr = mol_rep_map[name]
                            arr_t = torch.as_tensor(arr, dtype=torch.float32)
                            actual_length = arr_t.size(0)
                            feature_dim = arr_t.size(1) if arr_t.dim() > 1 else 0
                            if actual_length > 0:
                                if actual_length >= target_length:
                                    feat_t = arr_t[:target_length]
                                else:
                                    last_row = arr_t[actual_length - 1:actual_length]
                                    pad_rows = last_row.expand(target_length - actual_length, -1).clone()
                                    feat_t = torch.cat([arr_t, pad_rows], dim=0)
                            else:
                                feat_t = torch.zeros((target_length, feature_dim), dtype=torch.float32)
                            feats_torch.append(feat_t)
                        else:
                            if len(feats_torch) > 0:
                                feats_torch.append(torch.zeros_like(feats_torch[0]))
                            else:
                                feats_torch = None
                                break
                    if feats_torch is not None:
                        try:
                            mol_rep_ori = torch.stack(feats_torch, dim=0).to(mol_st.device)
                        except Exception:
                            mol_rep_ori = None
                ...
                for _, sample_pocket in enumerate(pocket_data):
                    ...
                    if pocket_rep_map is not None:
                        ...
                        if feats_torch is not None:
                            try:
                                pocket_rep_ori = torch.stack(feats_torch, dim=0).to(pocket_st.device)
                            except Exception:
                                pocket_rep_ori = None
                    mol_dist = sample["net_input"]["mol_src_distance"]
                    mol_et = sample["net_input"]["mol_src_edge_type"]
    ...
    def test_dude_target(self, target, model, **kwargs):
        ...
        # data_path_train = f"/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/{target}/mols_few{ft}_feature{suffix}.lmdb"
        # data_path_test = f"/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/{target}/mols_remain{ft}_feature{suffix}.lmdb"
        ...
        epoch = self.args.epoch_train
        mol_rep_map = None
        rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/dude/{target}.npz.pkl.gz"
        pocket_rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/dude/{target}.pockets.npz.pkl.gz"
    ...
    def test_pcba(self, model, **kwargs):
        ...
        df.to_csv(f'pcba_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}.csv', index=False)
    ...
    def test_dude(self, model, **kwargs):
        ...
        # print(suffix)
        for t in targets:
            lmdb_path = os.path.join(base_dir, t, f"mols_remain_{self.args.ft}pos_{self.args.ft}neg{suffix}.lmdb") #f"mols_remain_{self.args.ft}pos_{self.args.ft}neg.lmdb")
            if os.path.exists(lmdb_path):
                filtered_targets.append(t)
        ...
        # 保存为 CSV 文件
        df.to_csv(f'dude_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}.csv', index=False)
    ...
    def test_dekois_target(self, target, model, **kwargs):
        ...
        g.manual_seed(GLOBAL_SEED)
        suffix = f"_{self.args.sample_time}" if self.args.sample_time in [1, 2] else ""
        data_path_train = f"/root/autodl-tmp/Drug-fewshot/data/DEKOIS_2.0x/{target}/mols_few_{ft}pos_{ft}neg{suffix}.lmdb"
        data_path_test = f"/root/autodl-tmp/Drug-fewshot/data/DEKOIS_2.0x/{target}/mols_remain_{ft}pos_{ft}neg{suffix}.lmdb"
        ...
        pocket_dataset = self.load_pockets_dataset_fewshot(
            f"/root/autodl-tmp/Drug-fewshot/data/DEKOIS_2.0x/{target}/{target}_pocket.lmdb")
        pocket_data = torch.utils.data.DataLoader(
            pocket_dataset,
            batch_size=5,
            collate_fn=pocket_dataset.collater,
        ...
        epoch = self.args.epoch_train
        mol_rep_map = None
        rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/dekois/{target}.npz.pkl.gz"
        pocket_rep_path = f"/root/autodl-tmp/Drug-fewshot/embeddings/dekois/{target}.pockets.npz.pkl.gz"
    ...
    def test_dekois(self, model, **kwargs):
        ...
        # 保存为 CSV 文件
        df.to_csv(f'dekois_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}.csv', index=False)
