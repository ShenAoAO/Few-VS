...
#         #names = "PPARG"
#         # name = "GBA"
#         data_path = "./data/lit_pcba/" + name + "/mols_remain_50pos_50neg.lmdb"
...
#             mol_names.extend(sample["smi_name"][0].split()[-1])
...
#         pocket_data = torch.utils.data.DataLoader(pocket_dataset, batch_size=5, collate_fn=pocket_dataset.collater)
#         pocket_reps = []
...
#         print(f'target:{name}', f'auc:{auc}', f'bedroc:{bedroc}')
...
#         save_name = ""
#         targets = os.listdir("/root/autodl-tmp/Drug-fewshot/data/lit_pcba/")
#         model.load_state_dict(torch.load("binding_pose_220908.pt")["model"],strict = False)
#         # model.pocket_model.load_state_dict(torch.load("pocket_pre_220816.pt")["model"],strict = False)
#         # model.mol_model.load_state_dict(torch.load("mol_pre_no_h_220816.pt")["model"],strict = False)
...
#         # filtered_targets = []
#         # for t in targets:
#         #     lmdb_path = os.path.join(base_dir, t, "mols_remain_50pos_50neg.lmdb")
#         #     if os.path.exists(lmdb_path):
#         #         filtered_targets.append(t)
#         # # save filtered target names as npz
#         # np.savez(os.path.join(base_dir, "targets_with_50pos_50neg.npz"), targets=np.array(filtered_targets))
#         # targets = filtered_targets
#         for i,target in enumerate(targets):
#             out_path = self.save_target_mol_features(
#                 target,
#                 model=model,
#                 out_dir="/root/autodl-tmp/Drug-fewshot/embeddings_bd/pcba",
#                 backend="npz",      # 或 "lmdb"
#                 dtype="float16"     # 或 "float32"
#             )
#             print("saved to:", out_path)
#             out_path_p = self.save_target_pocket_features(
#                 target,
#                 model=model,
#                 out_dir="/root/autodl-tmp/Drug-fewshot/embeddings_bd/pcba",
#                 backend="npz",  # 或 "lmdb"
...
#         # # 保存为 CSV 文件
#         # df.to_csv('pcba_drugclip_target_metrics_30.csv', index=False)
...
#         data_path = "./data/dude/raw/all/" + target + "/mols_remain_100pos_100neg.lmdb"
#         mol_dataset = self.load_mols_dataset(data_path, "atoms", "coordinates")
#         num_data = len(mol_dataset)
#         bsz=64
#         print(num_data//bsz)
...
#         data_path = "./data/dude/raw/all/" + target + "/pocket.lmdb"
#         pocket_dataset = self.load_pockets_dataset(data_path)
...
#     #     base_dir = "./data/dude/raw/all/"
#     #     targets = os.listdir(base_dir)
#     #     filtered_targets = []
#     #     for t in targets:
#     #         lmdb_path = os.path.join(base_dir, t, "mols_remain_100pos_100neg.lmdb")
#     #         if os.path.exists(lmdb_path):
#     #             filtered_targets.append(t)
#     #     np.savez(os.path.join(base_dir, "targets_with_100pos_100neg.npz"), targets=np.array(filtered_targets))
#     #     targets = filtered_targets

#     #     auc_list = []
#     #     ef_list_summary = {"0.005": [], "0.01": [], "0.02": [], "0.05": []}
#     #     bedroc_list = []
#     #     re_list_summary = {"0.005": [], "0.01": [], "0.02": [], "0.05": []}

#     #     for target in targets:
#     #         auc, bedroc, ef, re, _, _ = self.test_dude_target(target, model)
#     #         auc_list.append(auc)
#     #         bedroc_list.append(bedroc)
#     #         for key in ef:
#     #             ef_list_summary[key].append(ef[key])
#     #         for key in re:
#     #             re_list_summary[key].append(re[key])

#     #     print(auc_list)
#     #     print(ef_list_summary)
#     #     print("auc 25%", np.percentile(auc_list, 25))
#     #     print("auc 50%", np.percentile(auc_list, 50))
#     #     print("auc 75%", np.percentile(auc_list, 75))
#     #     print("auc mean", np.mean(auc_list))
...
#     #     for key in ef_list_summary:
#     #         print("ef", key, "25%", np.percentile(ef_list_summary[key], 25))
#     #         print("ef", key, "50%", np.percentile(ef_list_summary[key], 50))
#     #         print("ef", key, "75%", np.percentile(ef_list_summary[key], 75))
#     #         print("ef", key, "mean", np.mean(ef_list_summary[key]))
#     #     for key in re_list_summary:
#     #         print("re", key, "25%", np.percentile(re_list_summary[key], 25))
#     #         print("re", key, "50%", np.percentile(re_list_summary[key], 50))
#     #         print("re", key, "75%", np.percentile(re_list_summary[key], 75))
#     #         print("re", key, "mean", np.mean(re_list_summary[key]))
...
#     #         'BEDROC': bedroc_list
#     #     })
#     #     df.to_csv('dude_drugclip_target_metrics.csv', index=False)
...
#         targets = os.listdir("/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/")
#         # 例如在任务内部
#         for i,target in enumerate(targets):
#             out_path = self.save_target_mol_features(
#                 target,
#                 model=model,
#                 out_dir="/root/autodl-tmp/Drug-fewshot/embeddings_bd/dude",
#                 backend="npz",      # 或 "lmdb"
#                 dtype="float16"     # 或 "float32"
#             )
#             print("saved to:", out_path)
...
#             )
#             print("saved to:", out_path_p)
#         # targets = torch.load('/mnt/sdb1/Drug-fewshot/targets_100.npz')
#         # auc_list = []
#         # bedroc_list = []
#         # ef_list = []
#         # res_list= []
...
#         # # 保存为 CSV 文件
#         # df.to_csv('target_metrics_100.csv', index=False)
...
#     def save_target_mol_features(self, target_name, model, out_dir, backend="npz", dtype="float16"):
#         """Compute and save all molecule features for a given target into a single file.

#         - Builds a Python dict: {smi_name: feature_vector}
#         - Saves to disk in one of the following formats:
#           - backend="npz": gzip-compressed pickle (smallest, simplest); filename: {target}.npz.pkl.gz
#           - backend="lmdb": LMDB database under directory {target}.lmdb (one entry per molecule)
#         """
#         os.makedirs(out_dir, exist_ok=True)

#         data_path = os.path.join("/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/", target_name, "mols.lmdb")
#         mol_dataset = self.load_mols_dataset(data_path, "atoms", "coordinates")
...
#                     mol_x, padding_mask=mol_padding_mask, attn_mask=graph_attn_bias
#                 )
#                 mol_emb = mol_outputs[0]
#                 # mol_emb = model.mol_project(mol_encoder_rep)
#                 # mol_emb = mol_emb / mol_emb.norm(dim=1, keepdim=True)

#                 # Move to CPU as desired dtype for storage
#                 if dtype == "float16":
#                     mol_emb_np = mol_emb.detach().cpu().to(torch.float16).numpy()
#                 elif dtype == "float32":
#                     mol_emb_np = mol_emb.detach().cpu().to(torch.float32).numpy()
#                 else:
#                     raise ValueError(f"Unsupported dtype: {dtype}")

#                 # Collect names per item in batch
#                 names = sample["smi_name"]
#                 for idx in range(mol_emb_np.shape[0]):
#                     # ensure string and keep the last token
#                     smi_name = str(names[idx])
#                     smi_key = smi_name
#                     features_dict[smi_key] = mol_emb_np[idx]

#         # Save
#         if backend == "npz":
#             out_path = os.path.join(out_dir, f"{target_name}.npz.pkl.gz")
#             with gzip.open(out_path, "wb") as f:
#                 pkl.dump(features_dict, f, protocol=4)
#             return out_path
#         elif backend == "lmdb":
#             lmdb_path = os.path.join(out_dir, f"{target_name}.mdb")
#             self.write_lmdb(features_dict, lmdb_path)
#             return lmdb_path
#         else:
#             raise ValueError(f"Unsupported backend: {backend}")

#     def save_target_pocket_features(self, target_name, model, out_dir, backend="npz", dtype="float16"):
#         """Compute and save all pocket features for a given target into a single file.

#         - Builds a Python dict: {pocket_name: feature_vector}
#         - Saves to disk in one of the following formats:
#           - backend="npz": gzip-compressed pickle; filename: {target}.pockets.npz.pkl.gz
#           - backend="lmdb": single-file LMDB: {target}.pockets.mdb with name-as-key
#         """
#         os.makedirs(out_dir, exist_ok=True)

#         data_path = os.path.join("/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/", target_name, "pocket.lmdb")
#         pocket_dataset = self.load_pockets_dataset(data_path)
#         dataloader = torch.utils.data.DataLoader(
...
#                     pocket_x, padding_mask=pocket_padding_mask, attn_mask=graph_attn_bias
#                 )
#                 pocket_emb = pocket_outputs[0]
#                 # pocket_emb = model.pocket_project(pocket_encoder_rep)
#                 # pocket_emb = pocket_emb / pocket_emb.norm(dim=1, keepdim=True)

#                 if dtype == "float16":
#                     pocket_emb_np = pocket_emb.detach().cpu().to(torch.float16).numpy()
#                 elif dtype == "float32":
#                     pocket_emb_np = pocket_emb.detach().cpu().to(torch.float32).numpy()
#                 else:
#                     raise ValueError(f"Unsupported dtype: {dtype}")

#                 names = sample["pocket_name"]
#                 for idx in range(pocket_emb_np.shape[0]):
#                     raw = names[idx]
...
#                     else:
#                         key = str(raw).strip()
#                     features_dict[key] = pocket_emb_np[idx]

#         if backend == "npz":
#             out_path = os.path.join(out_dir, f"{target_name}.pockets.npz.pkl.gz")
#             with gzip.open(out_path, "wb") as f:
#                 pkl.dump(features_dict, f, protocol=4)
#             return out_path
#         elif backend == "lmdb":
#             lmdb_path = os.path.join(out_dir, f"{target_name}.pockets.mdb")
#             self.write_lmdb(features_dict, lmdb_path)
#             return lmdb_path
#         else:
#             raise ValueError(f"Unsupported backend: {backend}")

#     # def write_lmdb(self, features_dict, lmdb_path, map_size_bytes=None):
#     #     """Write features dict to a single LMDB file, matching existing write_lmdb behavior.
#     #
#     #     - lmdb_path: single .mdb file path
#     #     - features_dict: dict with {str: np.ndarray} pairs
#     #     - map_size_bytes: optional, defaults to 1TB like your existing function
#     #     """
#     #     if not _HAS_LMDB:
#     #         raise RuntimeError("lmdb package not available. Install 'lmdb' or avoid LMDB backend.")
#     #
#     #     if map_size_bytes is None:
#     #         map_size_bytes = 1099511627776  # 1TB, same as your existing function
#     #
#     #     # 创建目录（如果不存在）
...
#     #             key_bytes = name.encode('utf-8') if not isinstance(name, bytes) else name
#     #             txn.put(key_bytes, pkl.dumps(vec, protocol=4))
#     #
#     #     env.close()
...
class DrugCLIP(UnicoreTask):
    ...
    def test_pcba_target(self, name, model, **kwargs):
        ...
        data_path = f"/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/{name}/mols_remain{ft}_scaffold{suffix}.lmdb"
        mol_dataset = self.load_mols_dataset(data_path, "atoms", "coordinates")
        num_data = len(mol_dataset)
        bsz=32
        #print(num_data//bsz)
    ...
    def test_pcba(self, model, **kwargs):
        ...
        suffix = f"_{self.args.sample_time}" if self.args.sample_time in [1, 2] else ""
        for t in targets:
            lmdb_path = os.path.join(base_dir, t, f"mols_remain{self.args.ft}_scaffold{suffix}.lmdb") #f"mols_remain_{self.args.ft}pos_{self.args.ft}neg.lmdb")
            if os.path.exists(lmdb_path):
                filtered_targets.append(t)
        np.savez(os.path.join("/root/autodl-tmp/Drug-fewshot/data/", f"pcba_targets_with_{self.args.ft}pos_{self.args.ft}neg.npz"),
        ...
        df.to_csv(f'pcba_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}_scaffold_drugclip.csv', index=False)
    ...
    def test_dude(self, model, **kwargs):
        ...
        #         target,
        #         model=model,
        #         out_dir="/root/autodl-tmp/Drug-fewshot/embeddings/dude",
        #         backend="npz",  # 或 "lmdb"
        #         dtype="float16"  # 或 "float32"
        #     )
        #     print("saved to:", out_path)
        ...
        # print(suffix)
        for t in targets:
            lmdb_path = os.path.join(base_dir, t, f"mols_remain{self.args.ft}_feature{suffix}.lmdb") #f"mols_remain_{self.args.ft}pos_{self.args.ft}neg.lmdb")
            if os.path.exists(lmdb_path):
                filtered_targets.append(t)
        np.savez(os.path.join("/root/autodl-tmp/Drug-fewshot/data/", f"dude_targets_with_{self.args.ft}pos_{self.args.ft}neg.npz"),
                 targets=np.array(filtered_targets))
        targets = filtered_targets
        # print(targets)
        auc_list = []
        ...
        # 保存为 CSV 文件
        df.to_csv(f'dude_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}_feature_drugclip.csv', index=False)
    ...
    def test_dude_target(self, target, model, **kwargs):
        ft = self.args.ft
        suffix = f"_{self.args.sample_time}" if self.args.sample_time in [1, 2] else ""
        data_path = f"/root/autodl-tmp/Drug-fewshot/data/dude/raw/all/{target}/mols_remain{ft}_feature{suffix}.lmdb"
    ...
    def test_dekois(self, model, **kwargs):
        base_dir = "/root/autodl-tmp/Drug-fewshot/data/DEKOIS_2.0x/"
        targets = os.listdir(base_dir)
        filtered_targets = []
        suffix = f"_{self.args.sample_time}" if self.args.sample_time in [1, 2] else ""
        for t in targets:
            lmdb_path = os.path.join(base_dir, t,
                                     f"mols_remain_{self.args.ft}pos_{self.args.ft}neg{suffix}.lmdb")  # f"mols_remain_{self.args.ft}pos_{self.args.ft}neg.lmdb")
            if os.path.exists(lmdb_path):
                filtered_targets.append(t)
        np.savez(os.path.join("/root/autodl-tmp/Drug-fewshot/data/",
                              f"dekois_targets_with_{self.args.ft}pos_{self.args.ft}neg.npz"),
                 targets=np.array(filtered_targets))
        targets = filtered_targets
        ...
        for target in targets:
            auc, bedroc, ef, re, res_single, labels = self.test_dekois_target(target, model)
            auc_list.append(auc)
            bedroc_list.append(bedroc)
            for key in ef:
                ef_list[key].append(ef[key])
        ...
        df.to_csv(
            f'dekois_target_metrics_{self.args.ft}_{self.args.lr}_{self.args.epoch_train}_fewshot_{self.args.sample_time}_moltoken{self.args.mol_token}_pockettoken{self.args.pocket_token}_{datetime.now():%Y%m%d-%H%M%S}_drugclip.csv',
            index=False)

        return

    def test_dekois_target(self, target, model, **kwargs):
        ft = self.args.ft

        # data_path = "./data/DUD-E/raw/all/" + target + "/mols.lmdb"
        # data_path = "./data/dude/raw/all/" + target + "/mols_remain_100pos_100neg.lmdb"
        suffix = f"_{self.args.sample_time}" if self.args.sample_time in [1, 2] else ""
        data_path = f"/root/autodl-tmp/Drug-fewshot/data/DEKOIS_2.0x/{target}/mols_remain_{ft}pos_{ft}neg{suffix}.lmdb"
        ...
        # generate pocket data
        # data_path = "./data/DUD-E/raw/all/" + target + "/pocket.lmdb"
        data_path = f"./data/DEKOIS_2.0x/{target}/{target}_pocket.lmdb"
        pocket_dataset = self.load_pockets_dataset(data_path)
        pocket_data = torch.utils.data.DataLoader(pocket_dataset, batch_size=bsz, collate_fn=pocket_dataset.collater)
        pocket_reps = []
