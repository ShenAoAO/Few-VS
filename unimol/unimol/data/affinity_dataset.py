...
class AffinityMolDataset_fewshot(BaseWrapperDataset):
    def __init__(
            self,
            dataset,
            seed,
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        added_atoms = np.concatenate([np.array(['<[UNK]>'] * self.token), atoms])
        zeros = np.zeros((self.token, 3))
        padded_coordinates = np.vstack([zeros, coordinates])

        smi = self.dataset[index]["smi"]
        return {
            "atoms": added_atoms,
            "coordinates": padded_coordinates.astype(np.float32),
            "holo_coordinates": padded_coordinates.astype(np.float32),  # placeholder
            "smi": smi,
            "ori_length": ori_length
        }
...
class AffinityPocketDataset_fewshot(BaseWrapperDataset):
    def __init__(
            self,
            dataset,
            seed,
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if self.pocket in self.dataset[index]:
            ...
            pocket = ""
        pocket_added_atoms = np.concatenate([np.array(['<[UNK]>'] * self.token), pocket_atoms])
        zeros = np.zeros((self.token, 3))
        pocket_padded_coordinates = np.vstack([zeros, pocket_coordinates])
        return {
            "pocket_atoms": pocket_added_atoms,
            "pocket_coordinates": pocket_padded_coordinates.astype(np.float32),
            "holo_pocket_coordinates": pocket_padded_coordinates.astype(np.float32),  # placeholder
            "pocket": pocket,
            "ori_length": ori_length
        }
